...
export class ReadManyDepositService {
    ...
    async handle(query) {
        ...
        if (query.filter["isRollOver"]) {
            try {
                query.filter["isRollOver"] = Boolean(query.filter["isRollOver"] === true || query.filter["isRollOver"] === 'true');
            }
            catch (e) { }
        }
        if (query.filter["isCashback"]) {
            try {
                query.filter["isCashback"] = Boolean(query.filter["isCashback"] === true || query.filter["isCashback"] === 'true');
            }
            catch (e) { }
        }
        let costumeFilter = {};
        ...
        const result = await depositRepository.aggregate(pipeline, query);
